...
- **`useBookingStore`** (`~/stores/booking`) - хранилище состояния бронирования
  - Используемые свойства (через `storeToRefs`):
    - `date` - выбранные даты заезда/выезда (`Ref<[Date, Date] | null>`)
    - `guests` - информация о гостях (`Ref<{ rooms: number; roomList: { adults: number; children: number; childrenAges: number[] }[] }>`)
      - `rooms` - количество комнат
      - `roomList` - массив комнат с количеством взрослых и детей
  - Используемые методы:
...
- Использует `date.value`, где `date` получен через `storeToRefs(bookingStore)`
- Реактивно обновляется при изменении дат в store

##### `totalAdults: number`

Вычисляет общее количество взрослых гостей из всех комнат.

**Тип:** `ComputedRef<number>`
...
- Использует `guests.value?.roomList`, где `guests` получен через `storeToRefs(bookingStore)`
- Реактивно обновляется при изменении данных о гостях в store
...
Форматирует минимальную цену номера для отображения.

**Алгоритм:**

- Берет `min_price` из `currentRoom.value`
- Если цена не `null` и не `undefined`, возвращает строку вида `"{цена} руб."`
- Иначе возвращает `null`

**Использование:** Отображается в блоке цен как "От {цена} руб."
...
Асинхронный метод обработки перехода к выбору тарифа. Содержит сложную логику валидации и определения корректного `room_type_code` для сохранения в store.
...
3. **Определение `room_type_code`** - критически важный этап с валидацией:

   **Сценарий 1: Есть варианты номеров (`room_type_codes`)**
   - Берется выбранный вариант (`currentRoom`) или первый из списка
   - Проверяется валидность кода:
     - Код не должен быть пустым или состоять только из пробелов
     - Код не должен совпадать с названием (`title`)
   - Если код валидный → используется
   - Если код невалидный → ищется первый вариант с валидным кодом
   - Если валидный код не найден → используется первый доступный код (fallback)

   **Сценарий 2: Нет вариантов (плоский формат)**
   - Используется код напрямую из `props.room.room_type_code`
   - Проверяется валидность кода (не пустой, не равен названию)
   - Если код невалидный → выводится ошибка в dev-режиме и выполнение прерывается

4. **Финальная валидация `room_type_code`:**
   - Проверяется, что код определен и не пустой
   - Проверяется, что код не равен названию номера
   - При неудаче выводится детальная ошибка в dev-режиме с информацией о:
     - Определенном коде
     - Данных текущего номера
     - Данных исходного номера из props
     - Списке всех вариантов
     - Выбранном типе кровати
   - Выполнение прерывается, если код невалиден

5. **Сохранение выбранного типа номера** - вызывает `bookingStore.setSelectedRoomType(roomTypeCode)` с валидированным кодом

6. **Определение количества комнат:**
   - Если есть `guests.value?.roomList`, берет его длину
   - Иначе берет `guests.value?.rooms` или `1`

   **Примечание:** Используется `guests.value`, так как `guests` получен через `storeToRefs(bookingStore)` и является реактивной ссылкой

7. **Определение целевого маршрута:**
   - Если комнат > 1 → `/multi-rooms`
   - Иначе → `/rooms/tariff`

8. **Навигация** - переходит на целевой маршрут через `router.push()`

9. **Сброс состояния** - в блоке `finally` устанавливает `loading.value = false`

**Валидация `room_type_code`:**

Метод выполняет строгую валидацию кода типа номера, так как:

- Код должен быть сокращенным идентификатором (например, "DLT"), а не полным названием
- Некорректный код может привести к ошибкам при запросах к API
- Сервер ожидает именно код, а не название номера

**Критерии валидности кода:**

- Код не должен быть `null` или `undefined`
- Код не должен быть пустой строкой или состоять только из пробелов
- Код не должен совпадать с названием номера (`title`)

**Обработка ошибок:**

- Ошибки валидации логируются только в dev-режиме (`import.meta?.env?.DEV`)
- При ошибке валидации выполнение прерывается до сохранения в store
...
Компонент поддерживает работу с вариантами номеров (разные типы кроватей для одного номера). При работе с вариантами выполняется строгая валидация `room_type_code` для каждого варианта.
...
### Валидация кода при работе с вариантами

При переходе к выбору тарифа (`handleTariff`) выполняется валидация кода выбранного варианта:

1. Проверяется код выбранного варианта (`currentRoom.room_type_code`)
2. Если код валиден (не пустой, не равен названию) → используется
3. Если код невалиден → ищется первый вариант с валидным кодом
4. Если валидный код не найден → используется первый доступный код (fallback)
5. Финальная проверка: если код все еще невалиден → выполнение прерывается

**Критерии валидности кода варианта:**

- Код не должен быть пустым или состоять только из пробелов
- Код не должен совпадать с названием варианта
...
- При переходе к тарифу проверяется валидность кода из `props.room.room_type_code`
- Если код невалиден → выполнение прерывается с ошибкой в dev-режиме

## Навигация

При нажатии на кнопку "Выбрать номер" компонент:

1. Валидирует даты бронирования (обязательно)
2. Определяет и валидирует `room_type_code` (критически важно)
3. Сохраняет выбранный тип номера в store через `setSelectedRoomType(roomTypeCode)` с валидированным кодом
4. Определяет целевой маршрут на основе количества комнат:
   - **Одна комната:** `/rooms/tariff`
   - **Несколько комнат:** `/multi-rooms`
5. Выполняет навигацию через `router.push()`

**Важно:**

- Навигация происходит только при валидных датах бронирования
- Навигация происходит только при валидном `room_type_code`
- При невалидном коде выполнение прерывается до сохранения в store и навигации

## Валидация

Компонент выполняет многоуровневую валидацию перед переходом к выбору тарифа:
...
2. **Валидация `room_type_code`:** Критически важная проверка корректности кода типа номера
   - Код должен быть определен (не `null`/`undefined`)
   - Код не должен быть пустой строкой или состоять только из пробелов
   - Код не должен совпадать с названием номера (`title`)
   - При невалидном коде выполнение прерывается до сохранения в store
   - В dev-режиме выводится детальная информация об ошибке

**Особенности валидации кода:**

- При работе с вариантами номеров проверяется код выбранного варианта
- Если код выбранного варианта невалиден, ищется первый валидный вариант
- При работе без вариантов проверяется код из `props.room`
- Валидация предотвращает отправку некорректных данных на сервер
...
Компонент выводит ошибки в консоль только в dev-режиме (`import.meta?.env?.DEV`). Логирование используется для отладки проблем с данными номеров.

### Ошибки валидации

1. **Отсутствие дат бронирования:**

```javascript
console.error("Не выбраны даты бронирования");
```

Выводится при попытке перехода к выбору тарифа без выбранных дат.

2. **Невалидный `room_type_code` в props (плоский формат):**
...
Выводится когда код типа номера в исходном объекте невалиден (пустой или равен названию).

3. **Не удалось определить валидный код типа номера:**
...
Выводится при финальной проверке, если код все еще невалиден после всех попыток его определения. Содержит полную диагностическую информацию для отладки.

**Примечание:** В production-режиме ошибки не логируются, но выполнение все равно прерывается при невалидных данных.
...
- Данные о датах и гостях синхронизируются с `bookingStore` через `storeToRefs()`
  - `date` и `guests` являются реактивными ссылками (`Ref`), полученными через `storeToRefs(bookingStore)`
  - Доступ к значениям осуществляется через `.value` (например, `date.value`, `guests.value`)
  - Изменения в store автоматически отражаются в компоненте благодаря реактивности
- Компонент автоматически определяет целевой маршрут на основе количества комнат из `guests.value`
...
- **Критически важно:** Компонент выполняет строгую валидацию `room_type_code` перед сохранением в store
  - Код должен быть сокращенным идентификатором (например, "DLT"), а не полным названием
  - Валидация предотвращает отправку некорректных данных на сервер
  - При невалидном коде выполнение прерывается с детальным логированием в dev-режиме
- Логирование ошибок происходит только в dev-режиме для отладки проблем с данными

## Типизация

### Типы из store

После использования `storeToRefs(bookingStore)`:

- **`date`**: `Ref<[Date, Date] | null>`
  - Реактивная ссылка на массив из двух дат (заезд и выезд) или `null`
  - Доступ: `date.value`

- **`guests`**: `Ref<{ rooms: number; roomList: { adults: number; children: number; childrenAges: number[] }[] }>`
  - Реактивная ссылка на объект с информацией о гостях
  - `rooms` - количество комнат
  - `roomList` - массив комнат с количеством взрослых и детей
  - Доступ: `guests.value.rooms`, `guests.value.roomList`

### Типы методов store

- **`setSelectedRoomType(roomTypeCode: string): void`**
  - Устанавливает выбранный тип номера в store
  - Очищает список тарифов (`roomTariffs.value = []`)
  - Используется перед переходом к выбору тарифа
