  ...
  if (isMultiRoomsMode.value) {
    ...
    const servicesTotal = Object.values(selectedMultiRooms.value).reduce(
      (sum, entry) =>
        sum +
        bookingStore
          .getSelectedServicesForRoom(entry.roomIdx)
...
const handleContinue = async () => {
  if (bookingStore.changeServicesUuid) {
    await router.push(`/confirmation?uuid=${bookingStore.changeServicesUuid}`);
    return;
  }
...
<template>
  <div :class="$style.container">
    ...
    <section :class="$style.block">
      <div v-if="selectedRoom || isMultiRoomsMode" :class="$style.twoCols">
        ...
        <div :class="$style.content">
          ...
          <section :class="$style.servicesList">
            ...
            <div
              v-if="isMultiRoomsMode && multiRoomTabIndices.length > 0"
              :class="$style.roomTabs"
            >
              <button
                ...
                :class="[
                  $style.roomTab,
                  activeRoomTab === tabIndex && $style.roomTabActive,
                ]"
                @click="activeRoomTab = tabIndex"
              >
                Номер
                {{
                  (multiRoomEntriesSorted[tabIndex]?.roomIdx ?? tabIndex) + 1
                }}
              </button>
            </div>
            <h2 v-else :class="$style.servicesListTitle">
              Дополнительные услуги
            </h2>
