<script setup lang="ts">
  ...
  const desktopImages = useImages("images/events/bg-slider");
  const mobileImages = useImages("images/events/bg-slider");
</script>
...
<template>
  ...
  >
    <template #left-column>
      <div>
        <h3 :class="$style.subTitle">Деловые и торжественные мероприятия</h3>
        <p :class="$style.text">
          в конференц-зале и переговорных комнатах с естественным освещением и
          видом на Московский концертный зал «Зарядье»
        </p>
      </div>
    </template>
